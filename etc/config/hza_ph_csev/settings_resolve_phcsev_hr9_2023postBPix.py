...
flags = {
    ...
    'hza_resolve_phcsev_hr9_2023postBPix_sf': (
        # electron veto
...
tnpParNomFit = [
    ...
    "acmsP[60,50,95]","betaP[0.05,0.01,0.4]","gammaP[0.1,-2,2]","peakP[87.0,82.0,90.0]",
    "acmsF[65,55,95]","betaF[0.05,0.05,0.4]","gammaF[0.1,-0.5,2]","peakF[87.0,82.0,90.0]",
    ]
_BARREL_FAIL_NOMINAL = (
    "sigmaP[1.4,0.2,8]",
    "acmsP[75,45,95]",
    ...
    "sigmaF[1.4,0.2,8]",
    "acmsF[45,20,95]",
    "betaF[0.01,0.0005,0.4]",
    "gammaF[0.05,-0.5,0.5]",
    "peakF[90.0,84.0,94.0]",
)
_ENDCAP_FAIL_NOMINAL = (
    "sigmaP[1.6,0.2,8]",
    "acmsP[75,45,95]",
    "betaP[0.03,0.001,0.4]",
    "gammaP[0.08,-2,2]",
...
# 2026-08-29 nominalFit bin01 failing:nBkgF 停在框架下界 0.5、背景只佔 0.1%,
# acmsF/betaF/gammaF 三個形狀參數卻全部撞界 —— 半個事件約束不了四個形狀參數,
# 它們只是在無意義的方向游走(covQual=3 但 status=720)。背景近乎為零時該釘住形狀。
tnpParNomFitByBin = dict(globals().get('tnpParNomFitByBin', {}))
tnpParNomFitByBin[1] = params_with_updates(
    tnpParNomFitByBin.get(1, tnpParNomFit),
    "acmsF[70.0]", "betaF[0.05]", "gammaF[0.05]", "peakF[89.0]",
)
