...
flags = {
    ...
    'hza_resolve_phcsev_lr9_2023preBPix_sf': (
        # electron veto
...
# bkg/puup/pudown 三個變體必須與 nominal 同分箱,否則系統誤差無法對照。
   { 'var' : 'event_nPV' , 'type': 'float', 'bins': [10,35,100] },
...
tnpParNomFit = [
    ...
    "meanF[-0,-8,5]","sigmaF[1,0.2,8]",
    "acmsP[60,39,95]","betaP[0.05,0.01,0.4]","gammaP[0.1,-2,2]","peakP[87.0,82.0,90.0]",
    "acmsF[60,35,95]","betaF[0.05,0.01,0.4]","gammaF[0.1,-0.5,2]","peakF[87.0,82.0,90.0]",
    ]
_PEAKY_FAIL_NOMINAL = (
    "sigmaP[2,0.2,8]",
    "acmsP[75,39,95]",
    "betaP[0.04,0.001,0.4]",
    "gammaP[0.08,-2,2]",
    "peakP[90.0,84.0,94.0]",
    "meanF[-0.2,-8,1]",
    "sigmaF[2,0.2,8]",
    "acmsF[55,25,95]",
    "betaF[0.02,0.0005,0.4]",
    "gammaF[0.08,-0.5,1]",
    "peakF[90.0,84.0,94.0]",
)
tnpParNomFitByBin = {
...
tnpParAltSigFit = [
    ...
    "acmsF[60,35,95]","betaF[0.04,0.01,0.4]","gammaF[0.1,-0.5,1]","peakF[89.0,82.0,90.0]",
    ]
...
_PEAKY_FAIL_ALTSIGBKG = (
    'sigmaP[1.6, 0.1, 4.0]',
    'sigmaP_2[1.6, 0.1, 4.0]',
    'sosP[0.30, 0.0, 2.0]',
    'alphaP[2.0, 1.0, 5.0]',
    'nP[1.0, 0.0, 6.0]',
    'alphaP_2[-0.02, -0.5, 0.05]',
    'meanF[-0.2, -2.0, 1.0]',
    'sigmaF[0.8, 0.1, 2.0]',
    'sigmaF_2[0.8, 0.1, 2.0]',
    'sosF[0.08, 0.0, 0.5]',
    'alphaF[2.0, 1.0, 5.0]',
    'nF[1.0, 0.0, 6.0]',
    'alphaF_2[-0.02, -0.5, 0.05]',
)
...
# ---------------------------------------------------------------------------
# 2026-09-12 擬合範圍實驗（A/B/B'）：**已撤銷，不要重做**
# ---------------------------------------------------------------------------
# 事實：這批是 Z->mumugamma，m(mumugamma) 被選擇釘在 Z 峰上 —— 質量直方圖 80 格
# (50-130) 裡只有 20 格有資料 (80.0-100.0)，而擬合範圍寫死在 histFitter.C 的
# 60-120，三分之二是空的。（對照組 phid_lowpt 是 Zee，50-130 全滿。）
#
# 試過兩個窗（用 fitUtils/tnpEGM_fitter 新增的 opt-in 開關 fitMassRange）：
#   B  = (80., 100.)  16 格中 13 成功。nominal / altBkg 這類**模板訊號**全炸，
#        NaN 出在 sigPass 而非背景：setFitRange 會改 x 變數本身的預設範圍，
#        收到 20 GeV 之後 RooFFTConvPdf 的摺積緩衝不夠，訊號 pdf 歸一化積分變 0。
#   B' = (78., 102.)  15/16 成功，nominal 全活，covQual 幾乎全面變 3，
#        altSig 的 nSigF 撞 0.5 絕對下界那個病也消失。
#
# **但 B' 沒有修好真正的判準 —— 四種擬合的效率仍然不一致：**
#     bin  A 的差距 -> B' 的差距
#     00   0.0883   -> 0.0751
#     01   0.0841   -> 0.0872  (更差)
#     02   0.1284   -> 0.0689
#     03   0.1804   -> 0.1817  (更差)
# 而且只把窗從 80-100 改成 78-102（差 2 GeV），bin00 的 nominal 效率就從 0.9234
# 跳到 0.9745、altSig 從 0.9081 跳到 0.9789。答案對窗的選擇這麼敏感，
# 代表它不是被修好了，而是根本沒有被資料決定。covQual=3 不能當成功。
#
# 真正的瓶頸是 MC 統計，不是擬合設定：
#     bin00 Fail 原始entries=3   bin01=20   bin02=3   bin03=**0（空的）**
# bin03 的 nominal/altBkg/altSigBkg 在 B' 之後仍然 covQual=0、nSigF 停在
# 0.9*nTotF 的初始值，因為訊號模板是空的；同時 eff_MC = 43.8/(43.8+0) = 1.0000
# 且無誤差，SF 分母也是壞的。任何擬合範圍或背景模型都救不了這個。
#
# 「重產」也無效：2026-09-13 做過 DAS 對帳，五個 era 的輸入資料集**全部 100%
# 處理完畢**（2022preEE 525/525、2022postEE 1682/1682、2023preBPix 506/506、
# 2023postBPix 312/312、2024 2975/2975）。差距來自樣本本身 —— 2024 用雙緲子
# 過濾的 DYto2Mu-2Jets，2022/2023 只有 inclusive DYto2L-2Jets（只有 1/3 是
# Z->mumu），而 DYJetsTo2Mu 在 Run3Summer22/23 的 campaign 裡不存在。
# 唯一能顯著加統計的是 DYGto2LG（DY+gamma，四個 era 都有），但它的光子 pT 譜
# 與 inclusive DY 的 FSR 不同，混用需要 stitching，否則會偏移 eff_MC。
# 使用者 2026-09-13 決定暫不往這個方向走。
#
# 所以擬合範圍維持預設（不設 fitMassRange），結果已還原成 A（baseline）。
# fitMassRange / tnpBkgModel 兩個開關留在框架裡，預設關閉、對既有 measurement
# 完全惰性（已用 elminiIso 2025 nominal b21 做過逐位元回歸測試）。
