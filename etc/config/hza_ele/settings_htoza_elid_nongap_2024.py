...
tnpParAltSigFitByBin = {
    ...
    # --- bin12 passing：2026-09-13 試過放開 alphaP，失敗，已撤回 ---
    # 觀察到的是 alphaP 停在下界 1.2（誤差 0.0065）、nBkgP=234248 是 nominalFit
    # 的 19613 的 12 倍、nSigP 比 nominal 少 9.8%。看起來像「背景吃掉峰」。
    # 於是把 alphaP 下界放到 0.5、sigmaP/sigmaP_2 地板降到 0.4/0.3。結果全面變差：
    #     峰區 d/tot   改前 0.993-1.016（最糟 pull 8.0）→ 改後 0.966-1.086（22.9）
    #     60.5 GeV     曲線 3730 → 4585（資料 2398），pull -26.9 → -44.2
    #     高質量尾     107-117 GeV 四個點的 pull 全部惡化約 25%
    #     EDM 0.334 → 9.94，covQual 3 → 2
    # 唯一「改善」的 nSigP 一致性（差 9.8% → 0.9%）是帳面搬移不是擬合變好：
    # nBkgP 從 234248 塌到 5.3 +- 1.7e+06（未定解），那 23 萬個事件被記到訊號去了。
    #
    # 機制上的教訓：背景被消滅後，低質量側的曲線**不降反升**（3730 → 4585）——
    # DSCB 的左尾接管了背景的角色，而且做得更差。也就是說 alphaP 撞界與背景過大
    # 是同一個自由度的兩種表現，放開 alphaP 只是把問題從背景搬到訊號尾巴。
    #
    # ⚠️ 改前的峰區殘差本來就是 0.993-1.016，已經很好。要再動這一格之前，先確認
    #    使用者看到的「矮而寬」指的是哪個成分 —— 很可能是峰下方那塊背景
    #    （改前 bkg 在 84.5 GeV 是 11688，約峰高的 5%），而不是總曲線的形狀。
    12: params_with_updates(
        tnpParAltSigFit,
        "meanP[-0.4,-5.0,5.0]",
        "sigmaP[2.0,0.7,6.0]",
