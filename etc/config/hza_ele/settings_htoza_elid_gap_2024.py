...
tnpParAltSigFitByBin = {
    ...
    1: params_with_updates(
        ...
        "meanP[-1.0,-5.0,5.0]",
        "sigmaP[3.8,0.8,7.5]",
        "sigmaP_2[1.8,0.3,6.0]",
        "alphaP[1.8,0.8,4.0]",
        "nP[0.8,0.0,8.0]",
        "sosP[1.5,0.0,6.0]",
        "meanF[-1.5,-5.0,3.0]",
        "sigmaF[2.2,0.8,6.0]",
        "sigmaF_2[0.8,0.3,4.0]",
        "alphaF[1.5,0.8,4.0]",
        "nF[0.2,0.0,4.0]",
        "sosF[0.4,0.0,3.0]",
        "acmsP[88.,30.,120.]",
...
tnpParAltSigBkgFitByBin = {
    ...
    1: params_with_updates(
        ...
        'sosP[0.30, 0.0, 1.2]',
        'alphaP_2[-0.02, -0.12, 0.]',
        'meanF[-1.0, -4.0, 2.0]',
        'sigmaF[1.6, 0.3, 4.0]',
        'sigmaF_2[1.2, 0.1, 4.0]',
        'sosF[0.25, 0.0, 1.2]',
        'alphaF[2.2, 1.2, 3.5]',
        'nF[0.2, 0.0, 1.0]',
        'alphaF_2[-0.04, -0.20, -0.005]',
    ),
    6: params_with_updates(
        tnpParAltSigBkgFit,
        # 2026-09-07：與 gap_2025 / gap_2026 的 bin6 逐字相同的病。8/23 為了 bin0/bin2
        # 加的 alphaF_2[-0.15, -0.005] 是**整段負值**，背景只能遞減；但 bin6 的資料
        # 往高質量端上升（60-120 GeV 從 4865 到 17652，3.6 倍），擬合因此貼死在
        # -0.005（誤差 0.0000）變成水平線：曲線 10996 -> 10198 幾乎不動，
        # 低端 -55%、高端 +73%，10 個 slice 超過 5 sigma。
        # bin0/bin2 需要遞減背景是對的，bin6 不是。上界跨過 0，並放開一起撞界的
        # sigmaF / sigmaF_2 / sosF。2025 與 2026 已用同一組修好（最大偏差各降到
        # 10% 附近），2024 當時漏掉。
        'meanF[-0.8, -3.0, 1.0]',
        'sigmaF[0.5, 0.1, 2.0]',
