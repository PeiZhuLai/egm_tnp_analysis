...
# flag to be Tested
flags = {
    'hza_dielleg23trigger_gap_2026_sf': '(passHltEle23Ele12CaloIdLTrackIdLIsoVLLeg1L1match == 1 && el_hltE23E12leg1_dR < 0.3)',
}
...
# ---------------------------------------------------------------------------
# 2026-08-29  bin09 altSigFit failing（eta -1.44..0.00, ET 27-35）
#
# 診斷:sigmaF 撞下界 0.7(訊號核心塌成 0.7 GeV,物理上不可能),acmsF=89.08 在 [45,90]
# 的 98%(背景 turn-on 被上界釘住)。後果是形狀兩頭都太肥、峰位又偏高 4 GeV:
#   資料峰 85-90 (19257)   曲線峰 90-95 (22079)
#   60-65  資料 178 / 曲線 1177  -85%      105-110 資料 335 / 曲線  576  -42%
#   85-90  資料 19257 / 曲線 14510 +33%     90-95  資料 18632 / 曲線 22079 -16%
# 核心塌到下界之後,整個峰形只能靠 sigmaF_2(1.70)撐,背景又被迫往兩側漫延去補。
#
# 修法:
#   - sigmaF 下界抬到 1.2,不讓核心塌到非物理的 0.7(0.7 是 base 的下界,不是最佳解)
#   - acmsF 上界從 90 開到 95,讓 turn-on 脫離界去找真正的位置
#   - meanF 給接近資料峰的起點(-2),涵蓋 -6..3
#   - gammaF 起點抬高,允許高質量端衰減得更快(目前 105-120 曲線一路高於資料 40-57%)
tnpParAltSigFitByBin = dict(globals().get('tnpParAltSigFitByBin', {}))
tnpParAltSigFitByBin[9] = params_with_updates(
    tnpParAltSigFit,
    "meanF[-2.0,-6.0,3.0]",
    "sigmaF[2.0,1.2,8.0]",
    "sigmaF_2[3.0,1.0,10.0]",
    "sosF[1.0,0.2,5.0]",
    "acmsF[80.,45.,95.]",
    "gammaF[0.15,0.002,1.5]",
)
...
for _b, _sides in {5: 'f', 6: 'f'}.items():
