...
tnpParAltSigFitByBin = {
    ...
    5: params_with_updates(
        ...
        # 2026-08-23: fail 側 11 個參數有 7 個同時撞界(meanF↓ sigmaF↑ sosF↑ alphaF↓ nF↓
        # acmsF↓ betaF↑)。資料的 failing 譜是單調下降、看不到 Z 峰(60:12626 -> 115:3471),
        # 失敗電子的 Z bump 又寬又左移、埋在連續譜裡,所以擬合同時想要更寬、更左、更軟的尾巴。
        # 只放寬撞界的那一側,另一側保持不動。
        "meanF[-1.0,-5.0,1.0]",
        "sigmaF[2.0,0.5,6.0]",
...
tnpParAltSigBkgFitByBin = {
    ...
    # 2026-08-23: fail 側 sigmaF 撞在 0.1 GeV 的下界、nF 撞 0 —— 0.1 GeV 的電子質量解析度
    # 物理上不可能,那個「訊號」其實是一根尖峰在吸統計漲落,並讓 nSigF 灌到 24890
    # (同一個 bin 的 altSigFit 只給 9956,兩者效率差 1.6 倍)。
    # 這裡把 sigmaF 的地板抬到物理值 —— 注意這不是「放寬撞界那側」,是主動排除非物理解,
    # 屬於判斷而非純粹依資料。
    5: params_with_updates(
        tnpParAltSigBkgFit,
        'meanF[-1.0, -5.0, 2.0]',
        'sigmaF[2.5, 0.8, 8.0]',   # 上界 4.0 又被撞到 -> 只放寬撞界那側(altSigFit 同 bin 也要 6.0 才夠)
