...
# flag to be Tested
flags = {
    'hza_elid_nongap_highpT_2026_sf': (
        '('
        + baseline_cut +
        '('
        # ele Et > 10 region
...
tnpParNomFitByBin = {
    0: params_with_updates(
        ...
        # 2026-08-22: 原 tune 把 turn-on 設在 Z 峰之上(acmsF 初始 95、上界 130),背景在窗內
        # 幾乎沒有貢獻,於是訊號用 sigmaF=14.35(上界 16)去吃掉整段連續譜,擬合回報背景只有
        # 6.3%。但 fail 資料是 ~250 事件/5GeV 的連續背景貫穿 60-120,Z 峰僅超出約 870 事件,
        # 真實背景約 75%。et 100-500 GeV 電子的解析度是 1-2 GeV,14 GeV 的訊號寬度不合理。
        # 把 turn-on 拉回窗左緣、訊號寬度收回物理值,讓兩者各司其職。
        "meanF[0.0,-5.0,5.0]",
        "sigmaF[2.0,0.5,5.0]",
        "acmsF[70.,40.,88.]",   # 78 撞上界 -> 只放寬撞界那側(下界維持 40)
