...
# flag to be Tested
flags = {
    'hza_dielleg12trigger_gap_2026_sf': '(passHltEle23Ele12CaloIdLTrackIdLIsoVLLeg2 == 1 && el_hltE23E12leg2_dR < 0.3)',
}
...
# bin13/14 (et 20-30, barrel, eff~1 clean Z, empty failing leg): huge-statistics
# passing fit stalls (edm~8e3, status -1, signal params frozen) because the bkg
# CMSShape turn-on acmsP rails at its upper bound (the Z peak). Fix: pin the bkg
# turn-on shape (acmsP/betaP/peakP) to constants and let meanP/sigmaP float to the
# data resolution -> converges (edm~1e-3, covQual=3).
_highpt_pin_bkg = (
    "meanP[0.0,-2.0,2.0]",
    "sigmaP[1.3,0.5,3.0]",
    "acmsP[60.0]",
...
# NOTE: low-pT (et 13-20) passing bins 5/9/10 are background-dominated with a
...
tnpParAltSigFitByBin = {
    6: params_with_updates(
        tnpParAltSigFit,
        # 2026-08-23: 與 dielleg12trigger_nongap_2026 bin08/09/15/16/17 同一個 bug 家族。
        # meanP[-30,-45,-8] 假設 passing 峰下移 30 GeV,但資料的峰就在 85-90
        # (660 576 609 969 1938 4453 4082 648 ...),最佳解在 -8 之外 -> meanP 死貼上界 -8,
        # sigmaP 則撞下界 3,七個參數同時撞界。那批 bin 放寬後收斂在 meanP = -1.5 ~ -2.6。
        # 沿用該處已驗證的參數組。
        "meanP[-2.0,-12.0,5.0]",
        "sigmaP[2.5,0.7,7.0]",
