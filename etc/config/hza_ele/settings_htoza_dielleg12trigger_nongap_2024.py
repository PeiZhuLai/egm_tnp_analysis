...
tnpParNomFitByBin = {
    ...
    # 2026-07-12: nominal bin46(endcap et50-100) failing 高質量尾未貼 + acmsF railed 90。略寬+解開acmsF。
    # 2026-07-13 user: failing 太寬把 MC template 高質量尾 smear 掉。收窄 sigmaF -> 少 smearing
    # -> template 高質量尾保留、fit 得上 100-120 tail。(acmsF 仍解開)
    46: params_with_updates(
        tnpParNomFit,
        "sigmaF[0.9,0.4,1.2]",
        "acmsF[70.,45.,88.]",
    ),
...
# 2026-07-12 夥伴回饋: bins 33/38(et30-50 endcap)、55(et100-500 far-endcap)
# passing signal 太窄(endcap 解析度較寬),紅線比 data 尖漏掉 shoulder + acmsP rail 在 90。
# widen passing signal core + 解開 acmsP 上限。
for _b in (33, 38, 55):
...
# 2026-07-12 夥伴回饋: altSig bin27(barrel et20-30) passing 峰高不足(sosP大攤平核心+acmsP railed吸峰)。
# 降第二Gaussian占比讓核心銳利匹配峰高 + acmsP綁低不吸峰。
tnpParAltSigFitByBin[27] = params_with_updates(
    tnpParAltSigFit,
    "meanP[0.0,-3.0,3.0]",
    "sigmaP[1.6,0.9,3.0]",
    "sigmaP_2[3.8,2.0,8.0]",
    "sosP[1.8,0.5,5.0]",
    "alphaP[1.5,0.8,3.0]",
    "nP[3.0,0.5,8.0]",
    "acmsP[70.,45.,85.]",
