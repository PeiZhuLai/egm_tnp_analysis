...
# flag to be Tested
flags = {
    'hza_dielleg12trigger_nongap_2026_sf': '(passHltEle23Ele12CaloIdLTrackIdLIsoVLLeg2 == 1 && el_hltE23E12leg2_dR < 0.3)',
}
...
# bin13: acmsF 貼在上界(89.5~90)，CMSShape 的 turn-on 被推到 Z 峰上，
...
# --- passing meanP 解除過度偏移 (2026-08-23) ---
# 2026-07-12 那次只放寬了 10-14,18-21,23,30,註解明載「未旗標的 8,9,15,16,17 保持 group
# 參數不動」。這五個仍受 group 的 meanP[-25.0,-40.0,-5.0] 約束,上界 -5 讓 meanP 不可能
# 接近 0。實測 passing 譜的 Z 峰就在 85-90 GeV (bin08 峰值 660@85-90、bin17 2846@85-90),
# 根本沒有下移 25 GeV;已放寬的手足 bin 收斂在 meanP = -2.2 ~ +0.4,全部落在 [-40,-5] 之外
# —— 最佳解被排除在範圍外,與前人 meanF[-25,-40,-5] 是同一個錯誤。
# 症狀: bin08 edm=1.24e+05、bin17 edm=2.01e+06 (bin09/15/16 則直接 railed 在 -5)。
# 這裡只動 passing 側 (使用者旗標的是 passing fit),參數沿用 2026-07-12 已驗證的那組。
# bin09/15/16 是同一批漏網:meanP 死貼在 -5.000 (=[-40,-5] 的上界),但 edm 1e-4~1e-5、
# covQual=3 全部正常 —— 任何以收斂為判準的檢查都抓不到,只有看「參數是否貼在人為邊界上」
# 才發現最佳解被排除在範圍外。2026-08-23 一併放寬,五個 bin 至此補齊。
tnpParAltSigFitByBin = dict(globals().get('tnpParAltSigFitByBin', {}))
for _b in (8, 9, 15, 16, 17):
    tnpParAltSigFitByBin[_b] = params_with_updates(
...
# --- bin07 passing background rises instead of falling (2026-09-06) ---
# alphaP came out +0.5466: tnpParAltBkgFit declares alphaP[0.,-5.,5.], so a
# rising exponential is allowed and this bin took it. With only 422 signal
# events the likelihood barely cares, and the signal went with it -- meanP on
# its -5 floor (+/- 7.9) and sigmaP on its 5.0 ceiling.
# Restrict alphaP to a falling background and give the signal physical bounds.
tnpParAltBkgFitByBin = dict(globals().get('tnpParAltBkgFitByBin', {}))
tnpParAltBkgFitByBin[7] = params_with_updates(
    tnpParAltBkgFitByBin.get(7, tnpParAltBkgFit),
    "alphaP[-0.03,-5.,0.]",
    "meanP[-1.0,-3.0,2.0]",
    "sigmaP[2.0,0.5,6.0]",
)
...
for _b, _sides in {23: 'fp', 24: 'fp', 31: 'fp'}.items():
...
for _b, _sides in {24: 'fp', 31: 'fp'}.items():
    _extra = _gaus_f + (_gaus_p if _sides == "fp" else ())
...
_bb2 = globals().get('tnpParAltSigFitByBin', {})
tnpParAltSigFit_addGausByBin = {}
for _b, _sides in {24: 'fp'}.items():
    _extra = _gaus_f + (_gaus_p if _sides == "fp" else ())
    tnpParAltSigFit_addGausByBin[_b] = params_with_updates(_bb2.get(_b, tnpParAltSigFit), *_extra)
