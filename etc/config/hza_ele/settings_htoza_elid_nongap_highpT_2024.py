...
# flag to be Tested
flags = {
    'hza_elid_nongap_highpT_2024_sf': (
        '('
        + baseline_cut +
        '('
        # ele Et > 10 region
...
#############################################################
biningDef = [
   { 'var' : 'el_sc_abseta' , 'type': 'float', 'bins': [0.0, 1.4442, 1.566, 2.5] },
   { 'var' : 'el_et' , 'type': 'float', 'bins': [100,500] },
]
...
tnpParNomFitByBin = {
    0: params_with_updates(
        ...
        # 2026-08-23: 與 elid_nongap_highpT_2026 bin00 完全同型(那組 tune 顯然是複製過來的)。
        # acmsF/betaF/sigmaF 三個都撞上界(130 / 0.12 / 16):turn-on 被推到質量窗之外,背景在
        # 窗內幾乎沒有貢獻,訊號只好用 sigmaF=15.9 去吃掉整段連續譜,擬合回報背景僅 4.5%。
        # 但 fail 資料是 ~1300 事件/5GeV 的連續背景貫穿 60-120,真實背景約 67%;
        # et 100-500 GeV 電子的解析度是 1-2 GeV,15.9 GeV 的訊號寬度不可能。
        # 2026 同名 bin 用這組參數修好:背景 6.3%->73.6%、sigmaF 14.3->3.04、效率 +6.50%。
        "meanF[0.0,-5.0,5.0]",
        "sigmaF[2.0,0.5,5.0]",
        "acmsF[70.,40.,88.]",
        "betaF[0.04,0.005,0.15]",
        "gammaF[0.04,-0.2,1.0]",
        "peakF[91.0,84.0,98.0]",
    ),
}
