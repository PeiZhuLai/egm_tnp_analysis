...
def main():
    ...
    for y in YEAR_LIST:
        ...
        # 依序處理 DY_MC_LO 與 DY_MC_NLO
        for tag, patt in [("DY_LO", "DY_MC_LO"), ("DY_NLO", "DY_MC_NLO")]:
            sample_key = next((k for k in sample_dict if patt in k), None)
            if not sample_key:
                print(f"[warn] 找不到 {y} 的 {patt} 樣本，略過")
                continue

            input_file = sample_dict[sample_key]['path']
            tree_path = "tnpPhoIDs/fitter_tree"
            chain = _build_chain(input_file, tree_path)
            total = chain.GetEntries()
            if total == 0:
                print(f"[error] {y}/{tag} 樣本事件數為 0: {input_file}")
                continue

            needed_branches = ["event_rho", "ph_sc_eta", "ph_et", "ph_chIso", "ph_neuIso", "ph_hoe"]
            missing = _check_branches(chain, needed_branches)
            if missing:
                print(f"[error] {y}/{tag} 缺少必要分支: {missing}")
                continue

            print(f"[info] === 開始繪圖: {y} {tag} (entries={total}) ===")
            for vraw, vtitle, nb, xmin, xmax, coeff_list in var_defs:
                vcorr = _corr_expr_piecewise(vraw, rho, bins, coeff_list)
                cname = f"c_{_sanitize(vraw)}_{YEAR}_{tag}"
                c = ROOT.TCanvas(cname, f"{vtitle} piecewise corrected {YEAR} {tag}", 700, 600)
                _draw_overlay(
                    chain,
                    expr_uncorr=vraw,
                    expr_corr=vcorr,
