...
RF() { local MOD=$1 WP=$2 ft=$3 b=$4; timeout 900 python3 -m egm_tnp_analysis.tnpEGM_fitter "$MOD" --flag "$WP" --doFit $ft --addGaus --fitSample mcNom --iBin $b; timeout 900 python3 -m egm_tnp_analysis.tnpEGM_fitter "$MOD" --flag "$WP" --doFit $ft --addGaus --fitSample data --iBin $b; }
echo "=== refit Joseph 0718 START $(date +%H:%M:%S) ==="
# 0p15_nongap_2024
M=$B.settings_htoza_elminiIso0p15_nongap_2024; W=hza_elminiIso0p15_nongap_2024_sf
RF $M $W "--altBkg" 15; RF $M $W "--altBkg" 26
RF $M $W "--altSig" 18; RF $M $W "--altSig" 20
RF $M $W "--altSigBkg" 20
