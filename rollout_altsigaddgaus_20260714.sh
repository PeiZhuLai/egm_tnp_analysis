...
CONFIGS=(
  elminiIso0p15_gap_2025 elminiIso0p15_nongap_2024 elminiIso0p15_nongap_2025
  elminiIso0p1_gap_2024 elminiIso0p1_gap_2025 elminiIso0p1_nongap_2024 elminiIso0p1_nongap_2025
)
echo "=== rollout altSig --addGaus 7 configs START $(date +%H:%M:%S) ==="
for c in "${CONFIGS[@]}"; do
  MOD=$B.settings_htoza_$c; WP=hza_${c}_sf
  echo "--- [$c] $(date +%H:%M:%S) ---"
  timeout 3000 $F $MOD --flag $WP --doFit --altSig --addGaus --fitSample mcNom || echo "FAIL mcNom $c"
  timeout 3000 $F $MOD --flag $WP --doFit --altSig --addGaus --fitSample data  || echo "FAIL data $c"
  $F $MOD --flag $WP --sumUp --exportJson || echo "FAIL sumUp $c"
  echo "--- DONE $c $(date +%H:%M:%S) ---"
done
echo "=== rollout END $(date +%H:%M:%S) ==="
