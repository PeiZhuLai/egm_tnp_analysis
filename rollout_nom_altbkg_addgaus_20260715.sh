...
echo "=== rollout nominal+altBkg --addGaus(解析) 8 configs START $(date +%H:%M:%S) ==="
for c in "${CONFIGS[@]}"; do
  MOD=$B.settings_htoza_$c; WP=hza_${c}_sf
  echo "--- [$c] $(date +%H:%M:%S) ---"
  timeout 3000 $F $MOD --flag $WP --doFit --fitSample mcNom --addGaus || echo "FAIL nomMC $c"
  timeout 3000 $F $MOD --flag $WP --doFit --fitSample data  --addGaus || echo "FAIL nomDat $c"
  timeout 3000 $F $MOD --flag $WP --doFit --altBkg --addGaus --fitSample mcNom || echo "FAIL abMC $c"
  timeout 3000 $F $MOD --flag $WP --doFit --altBkg --addGaus --fitSample data  || echo "FAIL abDat $c"
  $F $MOD --flag $WP --sumUp --exportJson || echo "FAIL sumUp $c"
  echo "--- DONE $c $(date +%H:%M:%S) ---"
done
echo "=== rollout nom+altBkg END $(date +%H:%M:%S) ==="
